<template>
    ...
    <div class="box fit q-pa-md contentProfessorDialog">
        ...
        <form @submit.prevent="save()">
            ...
            <div v-if="!vEdit" class="column text-bold text-h4 text-white q-gutter-y-md">
                ...
                <span>Nome: {{vProfessor.nome}}</span>
                <span>Matricula: {{vProfessor.matricula}}</span>
                <span>Email: {{vProfessor.email}}</span>
                <span>Senha: {{vProfessor.senha}}</span>
                <span>Materias: {{vProfessor.materia.map(m => m.nome).join(", ")}}</span>
                <span>Turmas: {{$store.state.turmas.filter(t => vProfessor.idTurmas.includes(t.idTurma)).map(t => t.nome).join(", ") }}</span>
            ...
            <div v-else class="column text-bold text-h4 q-gutter-y-md">
                ...
                <q-select outlined v-model="modelMateria" input-class="text-white" label-color="white" placeholder="Digite a(as) materia(as) do professor" :options="optionsMateria" label="Materias" use-chips use-input multiple />
...
import { Administrador, Materia, Professor, Turma } from 'src/@types/DB';
...
export default class ProfessorDialog extends Vue {

    @Prop() add!: boolean
    @Prop() professor!: Professor

    vAdd = this.add
    vEdit = this.add ? true : false

    vProfessor: Professor = this.professor

    modelNome = this.vProfessor.nome
    modelLogin = this.vProfessor.matricula
    modelSenha = this.vProfessor.senha
    modelEmail = this.vProfessor.email

    modelMateria: qSelectOptions[] = []
    ...
    created() {
        ...
        this.vProfessor.materia.forEach(m => { this.modelMateria.push({ label: m.nome, value: m }) })
        materias.forEach(m => { this.optionsMateria.push({ label: m.nome, value: m }) })

        this.vProfessor.idTurmas.forEach(t => { 
    ...
    async save() {
        ...
        if (this.vAdd) {
            ...
            this.vProfessor = await this.db.professor.create({
                ...
                materia: this.modelMateria.map(m => m.value as Materia),
            ...
            this.vProfessor = {
                ...
                idProfessor: this.vProfessor.idProfessor,
                nome: this.modelNome,
                senha: this.modelSenha,
                email: this.modelEmail,
                matricula: this.modelLogin,
            ...
            await this.db.professor.update(this.vProfessor)
    ...
    remove() {
        ...
        this.$swal.fire({
            ...
            title: "Excluir?",
            text: `Tem certaza que deseja excluir o professor '${this.vProfessor.nome}'?`,
            showConfirmButton: true,
            showDenyButton: true,
            confirmButtonText: "Sim",
            denyButtonText: "Não",
            target: ".contentProfessorDialog",
            customClass: {
            ...
            if (swal.isConfirmed) { this.db.professor.delete(this.vProfessor.idProfessor); this.hide() }
